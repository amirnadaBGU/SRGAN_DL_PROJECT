...
if __name__ == '__main__':
    ...
    generator_criterion = GeneratorLoss(generator_only=True)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for data, target in train_bar:
            ...
            (g_loss, image_loss,   adversarial_loss,  perception_loss,    tv_loss ,
                ...
                = generator_criterion(y, fake_img, real_img)
            g_loss.backward()
            optimizerG.step()

            # loss for current batch before optimization
            running_results['g_loss'] += g_loss.item() * batch_size # accumulate generator loss (total score over the epoch)
            running_results['d_loss'] += 0 * batch_size # accumulate discriminator loss (total score over the epoch)
            ...
            running_results['d_score'] += 0 * batch_size # accumulate discriminator's confidence on real images (total score over the epoch)
            running_results['g_score'] += 0 * batch_size # accumulate discriminator's confidence on fake images (total score over the epoch)
